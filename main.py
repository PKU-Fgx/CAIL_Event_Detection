...
class Augments:
    ...
    loss_save_path : str              = field(default="./loss.txt",                                                     metadata={ "help": "训练Loss的保存位置" })

    device : str                      = field(default="cuda:0",    metadata={ "help": "GPU idx" })
    batch_size : int                  = field(default=8,           metadata={ "help": "训练批次大小" })
    max_length : int                  = field(default=512,         metadata={ "help": "每一句话输入最长长度" })
    adam_epsilon : float              = field(default=1e-8,        metadata={ "help": "Epsilon for Adam optimizer." })
    warmup_proportion : float         = field(default=0.1,         metadata={ "help": "Warm-up 的比例" })
    lr : float                        = field(default=5e-6,        metadata={ "help": "学习率" })
    epoches : int                     = field(default=10,          metadata={ "help": "迭代次数" })
...
def train(config, mlm_model, dataloader, tokenizer):
    total_step = len(dataloader) * config.epoches
    ...
    for epoch in range(config.epoches):
        ...
        #     Saving     #
        # -------------- #
        mlm_model.save_pretrained(config.domain_pretrained_save_path + "Epoch[{}]".format(epoch))
        tokenizer.save_pretrained(config.domain_pretrained_save_path + "Epoch[{}]".format(epoch))
    
    with open(config.loss_save_path, "w") as f:
        f.write(loss_list)
...
def mlm():
    ...
    print("[2] 构建 Tokenizer 与 Model ...")
    tokenizer = AutoTokenizer.from_pretrained(config.pretrained_model_path)
    mlm_model = AutoModelForMaskedLM.from_pretrained(config.pretrained_model_path).to(config.device)
    ...
    train(config, mlm_model, dataloader, tokenizer)
